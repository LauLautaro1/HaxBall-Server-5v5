...
HaxballJS().then((HBInit) => {
    const room = HBInit({
        roomName: '💵​5V5 - BELLAVISTA💵​',
        maxPlayers: 10,
        public: true,
        noPlayer: true,
        token: "thr1.AAAAAGi2irRRy4vKQaRfng.JdaFnhSS3m0"
    });

    const fs = require('fs')

    // Vacía el archivo al iniciar el programa
    fs.writeFileSync('registro.txt', '');
    ...
    // registrar quien y cuando entra al server:

    function registrarEntrada(player) {
        const fecha = new Date();
        const fechaFormateada = fecha.toLocaleString();
        console.log(`✅ ${player.name} ha entrado al servidor en ${fechaFormateada}`);
        fs.appendFileSync('registro.txt', `✅ ${player.name} ha entrado al servidor en ${fechaFormateada}\n`);
    }
