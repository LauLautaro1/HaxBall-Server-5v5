...
class RTCDataChannel extends EventTarget {
  constructor(dataChannel, opts = {}) {
    ...
    __privateGet(this, _dataChannel).onMessage((data) => {
      if (ArrayBuffer.isView(data)) {
        data = this.binaryType === "arraybuffer" ? data.buffer : Buffer.from(data.buffer);
      }
  ...
  send(data) {
    if (__privateGet(this, _readyState) !== "open") {
      if (process.env.DATACHANNEL_SEND_DEBUG === "true") {
        console.error(
          new Exception.InvalidStateError(
  ...
  close() {
    if (__privateGet(this, _readyState) === "closing" || __privateGet(this, _readyState) === "closed") return;
